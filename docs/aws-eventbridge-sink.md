...
2. Build the connector in the `pulsar-io-aws-eventbridge` directory.
   ...
   ```bash
   ls target
   pulsar-io-aws-eventbridge-2.10.4.3-ht.nar
   ```
   ...
   ```json
    {
        ...
        "archive": "connectors/pulsar-io-aws-eventbridge-2.10.4.3-ht.nar",
        "tenant": "public",
        "namespace": "default",
        "inputs": [
...
spec:
  image: streamnative/pulsar-io-aws-eventbridge:2.10.4.3-ht
  replicas: 1
  maxReplicas: 1
  input:
    topics:
  ...
  resources:
    ...
    memory: 1G
  java:
    jar: connectors/pulsar-io-aws-eventbridge-2.10.4.3-ht.jar
...
2. Run the AWS EventBridge sink connector.
    ...
    --archive <pulsar-io-aws-eventbridge-2.10.4.3-ht.nar>
    ```
