...
2. Build the connector in the `pulsar-io-aws-eventbridge` directory.
   ...
   ```bash
   ls target
   pulsar-io-aws-eventbridge-3.0.0.4.nar
   ```
   ...
   ```json
    {
        ...
        "archive": "connectors/pulsar-io-aws-eventbridge-3.0.0.4.nar",
        "tenant": "public",
        "namespace": "default",
        "inputs": [
...
spec:
  image: streamnative/pulsar-io-aws-eventbridge:3.0.0.4
  replicas: 1
  maxReplicas: 1
  input:
    topics:
  ...
  resources:
    ...
    memory: 1G
  java:
    jar: connectors/pulsar-io-aws-eventbridge-3.0.0.4.jar
...
2. Run the AWS EventBridge sink connector.
    ...
    --archive <pulsar-io-aws-eventbridge-3.0.0.4.nar>
    ```
